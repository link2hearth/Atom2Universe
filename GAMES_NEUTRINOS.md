...
| **Trébuchet** | **10 à 50 par site rasé** (`NeutrinoRewards.trebuchet`), versé **une fois par site posé** : recommencer un site connu depuis l'état intact paie de nouveau — c'est le même travail — mais continuer à tirer sur des ruines déjà rasées ne paie rien. Le montant ne dépend **pas du nombre de tirs** : au trébuchet on règle sa machine sur plusieurs coups avant d'être sûr de toucher où l'on vise, et compter les tirs punirait le réglage. Il dépend de ce que le site oppose (`TargetGenerator.difficulty` : part de maçonnerie et nombre de pierres, à poids égal), le même chiffre qui fixe l'objectif de tirs du niveau. Hameau ≈ 15, château ≈ 34, métropole ≈ 42. |
