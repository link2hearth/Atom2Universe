...
class ArtistIconSearchActivity : ThemedActivity() {
    ...
    companion object {
        ...
        private const val TAG = "ArtistIconSearch"
    }
    ...
    private fun setupToolbar() {
        ...
        toolbar.title = getString(R.string.music_search_artist_icon)
    }
    ...
    private fun searchImages(artistName: String) {
        val query = "$artistName artist photo"
    ...
    private fun downloadImage(imageUrl: String) {
        ...
        lifecycleScope.launch {
            try {
                ...
                // Compress the image for artist icon (max 512px, max 100Ko)
                val compressedPath = if (filePath != null) {
                    compressForArtistIcon(filePath)
                } else null
    ...
    private fun createOutputFile(extension: String): File {
        ...
        val fileName = "artist_icon_$timestamp.$extension"
    ...
    /**
     * Compresse l'image telechargee pour une icone d'artiste.
     * Max 512px, max 100Ko.
     */
    private suspend fun compressForArtistIcon(inputPath: String): String? = withContext(Dispatchers.IO) {
        try {
            val inputFile = File(inputPath)
            val originalSize = inputFile.length()

            // Si l'image fait deja moins de 100Ko et est un JPEG, on peut la garder telle quelle
            // (mais on compresse quand meme pour s'assurer de la taille en pixels)
            val outputFile = File(cacheDir, "compressed_artist_icon_${System.currentTimeMillis()}.jpg")

            val success = ImageCompressor.compressArtistIcon(inputPath, outputFile.absolutePath)
            ...
            if (success && outputFile.exists()) {
                ...
                Log.d(TAG, "Artist icon compressed: ${originalSize / 1024}Ko -> ${newSize / 1024}Ko")
                outputFile.absolutePath
            } else {
                Log.e(TAG, "Failed to compress artist icon")
                null
            }
        } catch (e: Exception) {
            Log.e(TAG, "Error compressing artist icon", e)
            null
        }
    }
