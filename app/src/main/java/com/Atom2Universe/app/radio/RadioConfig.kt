    ...
    companion object {
        ...
        fun load(context: Context): RadioConfig {
            ...
            return try {
                ...
                val servers = json.optJSONArray("servers")?.toStringList()?.takeIf { it.isNotEmpty() }
                    ?: fallback.servers
                // Validation du timeout: min 1000ms, max 60000ms
                val rawTimeout = json.optLong("requestTimeoutMs", fallback.requestTimeoutMs)
                val requestTimeoutMs = rawTimeout.coerceIn(1000L, 60000L)
                val maxResults = json.optInt("maxResults", fallback.maxResults).coerceIn(1, 100)
        ...
        private fun defaultConfig(): RadioConfig {
            return RadioConfig(
                ...
                requestTimeoutMs = 15000,  // Bug 4.13: Augmenté de 8s à 15s pour éviter les timeouts
                maxResults = 25,
                hideBroken = true,
