...
/** Catalogue partagé par le hub et les statistiques. */
object GamesCatalog {
    val tiles: List<HubTile> = listOf(
        HubTile(
            id = "farm",
        ...
        HubTile(
            id = "motocross",
            titleRes = R.string.motocross_title,
        ...
        HubTile(
            id = "cosmorun",
            titleRes = R.string.cosmo_run_hub_title,
    ...
    fun orderedTiles(context: Context): List<HubTile> {
        val prefs = context.getSharedPreferences("games_hub_prefs", Context.MODE_PRIVATE)
        val ids = prefs.getString("tile_order", "").orEmpty().split(",").filter { it.isNotBlank() }.distinct()
        val ordered = ids.mapNotNull { id -> tiles.find { it.id == id } } + tiles.filter { it.id !in ids }
        // Même migration que le hub, sans modifier les préférences depuis les statistiques.
        if (prefs.getBoolean("accretion_before_chess", false)) return ordered
        val accretion = ordered.first { it.id == "bigger" }
        return ordered.filter { it.id != "bigger" }.toMutableList().apply {
            add(indexOfFirst { it.id == "chess" }, accretion)
        }
    }
}
