...
internal object SimplexNoise {
    ...
    init {
        ...
        for (i in 0..255) { perm[i] = p[i]; perm[i + 256] = p[i] }
    }
    ...
    fun noise(xin: Double, yin: Double, zin: Double): Double {
        val F3 = 1.0 / 3.0; val G3 = 1.0 / 6.0
        val s = (xin + yin + zin) * F3
        val i = floor(xin + s); val j = floor(yin + s); val k = floor(zin + s)
        val t = (i + j + k) * G3
        ...
        val x3 = x0-1+3*G3; val y3 = y0-1+3*G3; val z3 = z0-1+3*G3
        ...
        val n0 = if (t0 < 0) 0.0 else { t0 *= t0; t0*t0*dot(grad3[gi0], x0, y0, z0) }
        var t1 = 0.6 - x1*x1 - y1*y1 - z1*z1
        val n1 = if (t1 < 0) 0.0 else { t1 *= t1; t1*t1*dot(grad3[gi1], x1, y1, z1) }
        var t2 = 0.6 - x2*x2 - y2*y2 - z2*z2
        val n2 = if (t2 < 0) 0.0 else { t2 *= t2; t2*t2*dot(grad3[gi2], x2, y2, z2) }
        var t3 = 0.6 - x3*x3 - y3*y3 - z3*z3
        val n3 = if (t3 < 0) 0.0 else { t3 *= t3; t3*t3*dot(grad3[gi3], x3, y3, z3) }
